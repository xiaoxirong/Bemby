import { ref } from "vue";

type Locale = "zh" | "en";

const zh = {
  nav: {
    accounts: "账户",
    jobs: "任务",
    templates: "模板",
    settings: "设置",
    logs: "日志",
    help: "帮助",
    logout: "退出登录",
  },
  common: {
    save: "保存",
    saving: "保存中...",
    cancel: "取消",
    edit: "编辑",
    duplicate: "复制",
    delete: "删除",
    run: "运行",
    runBusy: "...",
    name: "名称",
    actions: "操作",
    yes: "是",
    no: "否",
    refresh: "刷新",
    loadMore: "加载更多",
    stop: "停止",
    stopping: "停止中...",
    blankForDefault: "留空使用系统默认",
    leaveBlankKeep: "留空保持不变",
    saveFailed: "保存失败",
    all: "全部",
    default: "默认",
    custom: "自定义",
    enable: "启用",
    disable: "禁用",
  },
  login: {
    subtitle: "登录管理面板",
    username: "用户名",
    password: "密码",
    signIn: "登录",
    signingIn: "登录中...",
    error: "用户名或密码错误",
    rateLimited: "登录尝试次数过多，请 15 分钟后重试",
    captcha: "验证码",
    captchaPlaceholder: "输入图中字符",
    captchaError: "验证码错误，请重试",
    captchaExpired: "验证码已过期，请刷新",
  },
  accounts: {
    title: "Telegram 账户",
    addBtn: "添加账户",
    colPhone: "手机号",
    colStatus: "状态",
    colAdded: "添加时间",
    noAccounts: "暂无账户",
    editTitle: "编辑账户",
    addTitle: "添加账户",
    labelName: "显示名称",
    labelPhone: "手机号码",
    labelApiId: "API ID",
    labelApiHash: "API Hash",
    apiHint: "从 my.telegram.org/apps 获取 API ID 和 Hash",
    authTitle: "认证",
    authHint: "验证码将发送到",
    labelCode: "验证码",
    labelTwoFa: "二步验证密码",
    sendCode: "发送验证码",
    sending: "发送中...",
    verify: "验证",
    verifying: "验证中...",
    submit: "提交",
    authenticate: "认证",
    confirmDelete: "确定删除此账户及其所有任务？",
    status: {
      unauthenticated: "未认证",
      pending_code: "等待验证码",
      pending_2fa: "等待二步验证",
      authenticated: "已认证",
    },
    errors: {
      sendFailed: "验证码发送失败",
      verifyFailed: "验证失败",
      twoFaFailed: "二步验证失败",
    },
  },
  jobs: {
    title: "任务",
    addBtn: "添加任务",
    nextRuns: "下次计划运行",
    colAccount: "账户",
    colType: "类型",
    colBotUrl: "机器人/网址",
    colBotUrlTpl: "机器人/网址/模板",
    colWindow: "时间窗口",
    colEnabled: "启用",
    noJobs: "暂无任务",
    allAccounts: "所有账户",
    allBotUrlTpl: "所有机器人/网址/模板",
    editTitle: "编辑任务",
    addTitle: "添加任务",
    labelName: "任务名称",
    labelType: "任务类型",
    labelAccount: "账户",
    selectAccount: "选择账户",
    labelBot: "机器人用户名",
    labelServerUrl: "服务器地址",
    labelEmbyUser: "Emby 用户名",
    labelEmbyPass: "Emby 密码",
    labelPlayDuration: "播放时长（秒）",
    labelUserAgent: "用户代理",
    uaDefault: "默认",
    uaCustom: "自定义...",
    labelStartCommand: "启动命令",
    startCommandHint: "支持占位符：{word:N} {WORD:N} {num:N} {alpha:N} {uuid}",
    labelMarkWatched: "播放后标记已看",
    markWatchedHint: "播放完成后，将该剧集标记为已看",
    labelProxy: "代理",
    proxyNone: "不使用代理",
    playbackRulesHint:
      "播放从剧集的随机 5–10% 位置开始，时长为设定时间加上 0–10% 的随机延长。",
    labelCheckinButton: "签到按钮文字",
    checkinButtonHint:
      "特殊选项：{anyBtn} 随机点击，{aiBtn} AI 智能选择（需配置 AI_API_KEY）",
    aiBtnOption: "{aiBtn} — AI 识别",
    anyBtnOption: "{anyBtn} — 随机",
    noApiKey: "未配置密钥",
    aiKeyWarning: "未配置 AI API 密钥，请前往设置页面配置。",
    aiHintPlaceholder: "例如：签到、查询信息、获取服务器状态…",
    aiHintHint: "可选：描述要做的事，AI 会据此选择按钮（留空则通用识别）",
    aiInputOption: "{aiInput} — AI 识别验证码",
    aiInputLengthPlaceholder: "例：4（可选）",
    aiInputLengthHint: "可选：验证码字符数，留空则自动识别",
    accountOptionalHint: "可选，用于发送通知",
    noAccount: "无（不发通知）",
    labelReplyTimeout: "回复超时（毫秒）",
    labelWindowStart: "开始时间（HHMM）",
    labelWindowEnd: "结束时间（HHMM）",
    labelMaxRetries: "最大重试次数",
    labelEnabled: "启用",
    confirmDelete: "确定删除此任务及其日志？",
    confirmDisable: "确定禁用此任务？",
    bulkEnable: "启用 ({n})",
    bulkDisable: "禁用 ({n})",
    bulkDelete: "删除 ({n})",
    confirmBulkDisable: "确定禁用所选的 {n} 个任务？",
    confirmBulkDelete: "确定删除所选的 {n} 个任务及其日志？此操作不可撤销。",
    selectedCount: "已选 {n} 个",
    extractToTemplate: "另存为模板",
    extractModalTitle: "从任务创建模板",
    extractTemplateName: "模板名称",
    extractConfirm: "创建并关联",
    errors: {
      nameRequired: "任务名称为必填项",
      accountRequired: "账户为必填项",
      hostRequired: "服务器主机为必填项",
      botRequired: "机器人用户名为必填项",
      embyCredRequired: "Emby 用户名和密码为必填项",
      customActionsRequired: "请至少添加一个动作",
    },
    custom: {
      actions: "动作链",
      addAction: "添加动作",
      noActions: "暂无动作，请添加至少一个。",
      labelTarget: "目标账户/机器人",
      actionSendCommand: "发送命令",
      actionWaitReply: "等待回复",
      actionDelay: "延时",
      actionClickButton: "点击按钮",
      actionEnterCaptcha: "输入验证码",
      labelContent: "消息/命令",
      contentHint: "支持占位符：{word:N} {WORD:N} {num:N} {alpha:N} {uuid}",
      labelMaxWait: "最长等待（毫秒）",
      labelWaitMs: "等待时长（毫秒）",
      labelButton: "按钮文字",
      buttonHint: "支持特殊值：{anyBtn} 随机，{aiBtn} AI 识别，{aiBtn:提示} 带提示的 AI 识别",
      labelMaxRetries: "最大重试次数",
      labelCaptchaLength: "验证码长度（可选）",
      labelSuccessContains: "成功包含文字（可选）",
      labelFailContains: "失败包含文字（可选）",
      successContainsPlaceholder: "例：签到成功",
      failContainsPlaceholder: "例：签到失败",
      successContainsHint: "收到含该文字的回复时标记成功，留空则任意回复均视为成功",
      failContainsHint: "收到含该文字的回复时标记失败，留空则不按文字判断",
      labelJobMaxRetries: "任务最大重试次数",
      jobMaxRetriesHint: "整个动作链失败后的重试次数，1 表示不重试",
    },
  },
  templates: {
    title: "模板",
    addBtn: "添加模板",
    noTemplates: "暂无模板",
    editTitle: "编辑模板",
    addTitle: "添加模板",
    labelName: "模板名称",
    colType: "类型",
    colBotUrl: "机器人/网址",
    colLinkedJobs: "关联任务",
    confirmDelete: "确定删除此模板？关联任务将被解绑，但不会被删除。",
    bulkEnable: "启用 ({n})",
    bulkDisable: "禁用 ({n})",
    bulkDelete: "删除 ({n})",
    confirmBulkDisable: "确定禁用所选的 {n} 个模板？",
    confirmBulkDelete: "确定删除所选的 {n} 个模板？关联任务将被解绑，但不会被删除。",
    colEnabled: "启用",
    linkedBadge: "来自模板",
    templateField: "由模板控制",
    labelTemplate: "模板",
    noTemplate: "无（不使用模板）",
    createFromTemplate: "从模板创建",
    shareBtn: "分享",
    shareSelectedBtn: "分享所选 ({n})",
    shareCopied: "已复制！",
    importBtn: "导入模板",
    importTitle: "导入模板",
    importLabel: "粘贴模板 JSON",
    importPlaceholder: '粘贴模板 JSON...',
    importError: "无效的模板 JSON",
    importConfirm: "导入",
  },
  logs: {
    title: "日志",
    allJobs: "所有任务",
    colTime: "时间",
    colJob: "任务",
    colAccount: "账户",
    colStatus: "状态",
    colMessage: "消息",
    noLogs: "暂无日志",
    filterPlaceholder: "搜索日志...",
    status: { success: "成功", failed: "失败", running: "运行中" },
    jobType: { checkin: "签到", embywatch: "观看", custom: "自定义" },
    showDevLogs: "显示开发者日志",
    devLogsLabel: "开发者",
    retire: "移除",
    unretire: "取消移除",
    showRetired: "显示已移除",
    retiredLabel: "已移除",
    aiPrompt: "AI 提示词",
    aiResponse: "AI 响应",
    aiRetries: "AI 重试记录",
    debug: {
      open: "AI 调试",
      title: "AI 调试 / 重放",
      promptPlaceholder: "输入提示词…",
      model: "模型",
      modelDefault: "— 使用默认模型 —",
      maxTokens: "最大 Token 数",
      run: "运行",
      running: "运行中…",
      close: "关闭",
      response: "AI 回复",
    },
    detail: {
      attempt: "第 {n} 次尝试",
      commandSent: "发送命令",
      botResponse: "机器人回复",
      buttonClicked: "点击按钮",
      callbackAnswer: "回调消息",
      noDetail: "无详细记录",
      noText: "（无文字内容）",
      availableButtons: "可用按钮",
      loading: "加载中...",
    },
    embyDetail: {
      runtime: "总时长",
      start: "开始位置",
      end: "结束位置",
      watched: "观看时长",
      markedWatched: "已标记已看",
      yes: "是",
      no: "否",
    },
    dev: {
      timing: "计时",
      totalMs: "总耗时",
      connectMs: "TG 连接",
      replyLatencyMs: "回复延迟",
      buttonClickMs: "按钮 API",
      buttonResponseMs: "按钮响应",
      errorName: "错误类型",
      msgCount: "消息数",
      responseSource: "响应来源",
      retryCount: "重试次数",
    },
  },
  settings: {
    title: "设置",
    sysDefaults: "系统默认值",
    saved: "设置已保存",
    saveFailed: "保存失败",
    labelTimezone: "默认时区",
    labelMaxRetries: "默认最大重试次数",
    labelDailyRun: "每天仅运行一次",
    dailyRunHint: "测试时可关闭此选项，允许任务在当天重复运行。",
    embyDefaults: "Emby 观看默认值",
    labelPlayDuration: "默认播放时长（秒）",
    labelDeviceName: "设备名称",
    labelUserAgent: "默认用户代理",
    uaPresetsSection: "用户代理预设",
    uaPresetName: "名称",
    uaPresetValue: "用户代理字符串",
    addPreset: "添加",
    uaPresetDeleteTip: "删除",
    proxiesSection: "代理服务器",
    proxiesHint: "在模板中选择代理，用于无法直连的 Emby 服务器。",
    proxyName: "名称",
    proxyUrl: "URL（如 http://user:pass@host:port）",
    addProxy: "添加",
    proxyDeleteTip: "删除",
    saveBtn: "保存",
    aiSection: "AI 按钮识别",
    aiHint: "用于 {aiBtn} 功能，从内联键盘中自动识别签到按钮。",
    aiProviderLabel: "推荐服务商：",
    aiProviderSuffix: "——免费注册后即可获取 API 密钥。",
    labelAiBaseUrl: "API 地址",
    labelAiApiKey: "API 密钥",
    labelAiModel: "模型",
    labelAiTimeout: "超时（毫秒）",
    aiProvidersSection: "AI 服务商",
    addProvider: "添加服务商",
    supplierName: "名称",
    supplierBaseUrl: "Base URL",
    supplierApiKey: "API 密钥",
    supplierTimeout: "超时（毫秒）",
    supplierModels: "模型",
    addModel: "添加模型",
    modelId: "模型 ID",
    defaultModel: "默认模型",
    defaultModelNone: "— 未选择 —",
    saveDefaultModel: "保存默认模型",
    noSuppliers: "尚未配置服务商",
    supplierNoApiKey: "未配置 API 密钥 — 点击编辑添加",
    deleteSupplier: "删除",
    editSupplier: "编辑",
    cancelEdit: "取消",
    saveSupplier: "保存",
    notifySection: "Telegram 通知",
    notifyHint:
      "任务结束时通过关联账户向该 Telegram 用户发送通知。支持用户名、@用户名 或 t.me 链接。",
    labelNotifyUsername: "通知目标",
    notifyUsernamePlaceholder: "@用户名、username 或 https://t.me/username",
    labelNotifyEvents: "通知时机",
    notifyEventFailed: "失败",
    notifyEventSuccess: "成功",
    adminCreds: "管理员",
    credSaved: "已更新，如更改了用户名或密码请重新登录。",
    credFailed: "更新失败",
    labelNewUsername: "新用户名",
    labelNewPassword: "新密码",
    labelCurrentPass: "当前密码",
    hintKeepBlank: "留空保持不变",
    currentPassRequired: "请输入当前密码",
    updateBtn: "更新",
    importExport: {
      title: "导入 / 导出",
      exportBtn: "导出数据",
      exportHint: "导出所有账户、任务和设置（含敏感信息）",
      exportWarning:
        "导出文件包含敏感信息（Telegram 会话 Token、API Hash、API 密钥等）。请妥善保管，切勿分享。是否继续导出？",
      importBtn: "导入",
      importLabel: "选择备份文件（.json）",
      importMode: "导入模式",
      modeMerge: "合并",
      modeMergeHint: "保留现有数据，新账户（按手机号去重）和任务追加进来",
      modeReplace: "替换全部",
      modeReplaceHint: "清空现有账户和任务后再导入",
      replaceWarning: "替换模式将删除所有现有账户和任务。是否继续？",
      importing: "导入中...",
      importSuccess: "导入完成：账户 {a} 个，模板 {t} 个，任务 {j} 个，设置 {s} 项",
      importFailed: "导入失败",
      noFileSelected: "请选择文件",
      invalidFile: "文件格式无效",
    },
  },
};

const en: typeof zh = {
  nav: {
    accounts: "Accounts",
    jobs: "Jobs",
    templates: "Templates",
    settings: "Settings",
    logs: "Logs",
    help: "Help",
    logout: "Logout",
  },
  common: {
    save: "Save",
    saving: "Saving...",
    cancel: "Cancel",
    edit: "Edit",
    duplicate: "Duplicate",
    delete: "Delete",
    run: "Run",
    runBusy: "...",
    name: "Name",
    actions: "Actions",
    yes: "Yes",
    no: "No",
    refresh: "Refresh",
    loadMore: "Load more",
    stop: "Stop",
    stopping: "Stopping...",
    blankForDefault: "— blank uses system default",
    leaveBlankKeep: "— leave blank to keep current",
    saveFailed: "Save failed",
    all: "All",
    default: "Default",
    custom: "Custom",
    enable: "Enable",
    disable: "Disable",
  },
  login: {
    subtitle: "Sign in to access the admin panel",
    username: "Username",
    password: "Password",
    signIn: "Sign in",
    signingIn: "Signing in...",
    error: "Invalid username or password",
    rateLimited: "Too many login attempts. Please try again in 15 minutes.",
    captcha: "Captcha",
    captchaPlaceholder: "Enter characters shown",
    captchaError: "Incorrect captcha, please try again",
    captchaExpired: "Captcha expired, please refresh",
  },
  accounts: {
    title: "Telegram Accounts",
    addBtn: "Add Account",
    colPhone: "Phone",
    colStatus: "Status",
    colAdded: "Added",
    noAccounts: "No accounts yet",
    editTitle: "Edit Account",
    addTitle: "Add Account",
    labelName: "Display Name",
    labelPhone: "Phone Number",
    labelApiId: "API ID",
    labelApiHash: "API Hash",
    apiHint: "Get API ID & Hash from my.telegram.org/apps",
    authTitle: "Authenticate",
    authHint: "A verification code will be sent to",
    labelCode: "Verification Code",
    labelTwoFa: "2FA Password",
    sendCode: "Send Verification Code",
    sending: "Sending...",
    verify: "Verify",
    verifying: "Verifying...",
    submit: "Submit",
    authenticate: "Authenticate",
    confirmDelete: "Delete this account and all its jobs?",
    status: {
      unauthenticated: "Unauthenticated",
      pending_code: "Pending Code",
      pending_2fa: "Pending 2FA",
      authenticated: "Authenticated",
    },
    errors: {
      sendFailed: "Failed to send code",
      verifyFailed: "Verification failed",
      twoFaFailed: "2FA failed",
    },
  },
  jobs: {
    title: "Jobs",
    addBtn: "Add Job",
    nextRuns: "Next Scheduled Runs",
    colAccount: "Account",
    colType: "Type",
    colBotUrl: "Bot / URL",
    colBotUrlTpl: "Bot / URL / Template",
    colWindow: "Window",
    colEnabled: "Enabled",
    noJobs: "No jobs yet",
    allAccounts: "All accounts",
    allBotUrlTpl: "All Bots / URLs / Templates",
    editTitle: "Edit Job",
    addTitle: "Add Job",
    labelName: "Job Name",
    labelType: "Job Type",
    labelAccount: "Account",
    selectAccount: "Select account",
    labelBot: "Bot Username",
    labelServerUrl: "Server URL",
    labelEmbyUser: "Emby Username",
    labelEmbyPass: "Emby Password",
    labelPlayDuration: "Play Duration (s)",
    labelUserAgent: "User Agent",
    uaDefault: "Default",
    uaCustom: "Custom...",
    labelStartCommand: "Start Command",
    startCommandHint:
      "Placeholders: {word:N} {WORD:N} {num:N} {alpha:N} {uuid}",
    labelMarkWatched: "Mark as watched after playback",
    markWatchedHint:
      "After playback completes, mark the episode as watched in Emby",
    labelProxy: "Proxy",
    proxyNone: "No proxy",
    playbackRulesHint:
      "Playback starts at a random 5–10% into the episode. Duration is the set time plus 0–10% random extra.",
    labelCheckinButton: "Check-in Button Text",
    checkinButtonHint:
      "Special: {anyBtn} picks random, {aiBtn} uses AI (requires AI_API_KEY)",
    aiBtnOption: "{aiBtn} — AI picks",
    anyBtnOption: "{anyBtn} — random",
    noApiKey: "no API key",
    aiKeyWarning: "AI API key not configured — set it in Settings.",
    aiHintPlaceholder: "e.g. checkin, get server info, query balance…",
    aiHintHint: "Optional: describe the task so AI picks the right button (leave blank for generic detection)",
    aiInputOption: "{aiInput} — AI reads captcha",
    aiInputLengthPlaceholder: "e.g. 4 (optional)",
    aiInputLengthHint: "Optional: expected captcha length, leave blank for auto",
    accountOptionalHint: "optional, for notifications",
    noAccount: "None (no notifications)",
    labelReplyTimeout: "Reply Timeout (ms)",
    labelWindowStart: "Window Start (HHMM)",
    labelWindowEnd: "Window End (HHMM)",
    labelMaxRetries: "Max Retries",
    labelEnabled: "Enabled",
    confirmDelete: "Delete this job and its logs?",
    confirmDisable: "Disable this job?",
    bulkEnable: "Enable ({n})",
    bulkDisable: "Disable ({n})",
    bulkDelete: "Delete ({n})",
    confirmBulkDisable: "Disable {n} selected jobs?",
    confirmBulkDelete: "Delete {n} selected jobs and their logs? This cannot be undone.",
    selectedCount: "{n} selected",
    extractToTemplate: "Save as Template",
    extractModalTitle: "Create Template from Job",
    extractTemplateName: "Template Name",
    extractConfirm: "Create and Link",
    errors: {
      nameRequired: "Job name is required",
      accountRequired: "Account is required",
      hostRequired: "Server host is required",
      botRequired: "Bot username is required",
      embyCredRequired: "Emby username and password are required",
      customActionsRequired: "Add at least one action",
    },
    custom: {
      actions: "Action chain",
      addAction: "Add action",
      noActions: "No actions yet. Add at least one.",
      labelTarget: "Target account / bot",
      actionSendCommand: "Send command",
      actionWaitReply: "Wait for reply",
      actionDelay: "Delay",
      actionClickButton: "Click button",
      actionEnterCaptcha: "Enter captcha",
      labelContent: "Message / command",
      contentHint: "Supports placeholders: {word:N} {WORD:N} {num:N} {alpha:N} {uuid}",
      labelMaxWait: "Max wait (ms)",
      labelWaitMs: "Wait duration (ms)",
      labelButton: "Button text",
      buttonHint: "Special values: {anyBtn} random, {aiBtn} AI pick",
      labelMaxRetries: "Max retries",
      labelCaptchaLength: "Captcha length (optional)",
      labelSuccessContains: "Success contains (optional)",
      labelFailContains: "Fail contains (optional)",
      successContainsPlaceholder: "e.g. check-in successful",
      failContainsPlaceholder: "e.g. check-in failed",
      successContainsHint: "Marks action success when reply contains this text; leave empty to treat any reply as success",
      failContainsHint: "Marks action failed when reply contains this text; leave empty for no text-based failure",
      labelJobMaxRetries: "Job max retries",
      jobMaxRetriesHint: "How many times to retry the whole action chain on failure; 1 means no retry",
    },
  },
  templates: {
    title: "Templates",
    addBtn: "Add Template",
    noTemplates: "No templates yet",
    editTitle: "Edit Template",
    addTitle: "Add Template",
    labelName: "Template Name",
    colType: "Type",
    colBotUrl: "Bot / URL",
    colLinkedJobs: "Linked Jobs",
    confirmDelete: "Delete this template? Linked jobs will be unlinked but not deleted.",
    bulkEnable: "Enable ({n})",
    bulkDisable: "Disable ({n})",
    bulkDelete: "Delete ({n})",
    confirmBulkDisable: "Disable {n} selected templates?",
    confirmBulkDelete: "Delete {n} selected templates? Linked jobs will be unlinked but not deleted.",
    colEnabled: "Enabled",
    linkedBadge: "From template",
    templateField: "Controlled by template",
    labelTemplate: "Template",
    noTemplate: "None (no template)",
    createFromTemplate: "Create from template",
    shareBtn: "Share",
    shareSelectedBtn: "Share Selected ({n})",
    shareCopied: "Copied!",
    importBtn: "Import Template",
    importTitle: "Import Template",
    importLabel: "Paste template JSON",
    importPlaceholder: "Paste template JSON...",
    importError: "Invalid template JSON",
    importConfirm: "Import",
  },
  logs: {
    title: "Logs",
    allJobs: "All jobs",
    colTime: "Time",
    colJob: "Job",
    colAccount: "Account",
    colStatus: "Status",
    colMessage: "Message",
    noLogs: "No logs yet",
    filterPlaceholder: "Search logs...",
    status: { success: "Success", failed: "Failed", running: "Running" },
    jobType: { checkin: "Check-in", embywatch: "Emby Watch", custom: "Custom" },
    showDevLogs: "Show developer logs",
    devLogsLabel: "DEV",
    retire: "Retire",
    unretire: "Unretire",
    showRetired: "Show retired",
    retiredLabel: "Retired",
    aiPrompt: "AI prompt",
    aiResponse: "AI response",
    aiRetries: "AI retries",
    debug: {
      open: "AI debug",
      title: "AI Debug / Replay",
      promptPlaceholder: "Enter prompt…",
      model: "Model",
      modelDefault: "— use default model —",
      maxTokens: "Max tokens",
      run: "Run",
      running: "Running…",
      close: "Close",
      response: "AI response",
    },
    detail: {
      attempt: "Attempt {n}",
      commandSent: "Command sent",
      botResponse: "Bot response",
      buttonClicked: "Button clicked",
      callbackAnswer: "Callback message",
      noDetail: "No detail recorded",
      noText: "(no text content)",
      availableButtons: "Available buttons",
      loading: "Loading...",
    },
    embyDetail: {
      runtime: "Runtime",
      start: "Start",
      end: "End",
      watched: "Watched",
      markedWatched: "Marked watched",
      yes: "Yes",
      no: "No",
    },
    dev: {
      timing: "Timing",
      totalMs: "Total",
      connectMs: "TG Connect",
      replyLatencyMs: "Reply Latency",
      buttonClickMs: "Button Click API",
      buttonResponseMs: "Button Response",
      errorName: "Error Type",
      msgCount: "Messages",
      responseSource: "Response Via",
      retryCount: "Retries",
    },
  },
  settings: {
    title: "Settings",
    sysDefaults: "System Defaults",
    saved: "Settings saved.",
    saveFailed: "Save failed",
    labelTimezone: "Default Timezone",
    labelMaxRetries: "Default Max Retries",
    labelDailyRun: "Enforce one run per day",
    dailyRunHint:
      "Disable when testing so the scheduler re-runs even if the job already ran today.",
    embyDefaults: "Emby Watch Defaults",
    labelPlayDuration: "Default Play Duration (seconds)",
    labelDeviceName: "Device Name",
    labelUserAgent: "Default User Agent",
    uaPresetsSection: "User Agent Presets",
    uaPresetName: "Name",
    uaPresetValue: "User Agent string",
    addPreset: "Add",
    uaPresetDeleteTip: "Delete",
    proxiesSection: "Proxies",
    proxiesHint: "Select a proxy on a template to reach Emby servers that are blocked from direct access.",
    proxyName: "Name",
    proxyUrl: "URL (e.g. http://user:pass@host:port)",
    addProxy: "Add",
    proxyDeleteTip: "Delete",
    saveBtn: "Save",
    aiSection: "AI Button Detection",
    aiHint:
      "Used by {aiBtn} to automatically identify the check-in button from an inline keyboard.",
    aiProviderLabel: "Recommended provider:",
    aiProviderSuffix: "— sign up for a free API key.",
    labelAiBaseUrl: "API Base URL",
    labelAiApiKey: "API Key",
    labelAiModel: "Model",
    labelAiTimeout: "Timeout (ms)",
    aiProvidersSection: "AI Providers",
    addProvider: "Add Provider",
    supplierName: "Name",
    supplierBaseUrl: "Base URL",
    supplierApiKey: "API Key",
    supplierTimeout: "Timeout (ms)",
    supplierModels: "Models",
    addModel: "Add Model",
    modelId: "Model ID",
    defaultModel: "Default Model",
    defaultModelNone: "— select a model —",
    saveDefaultModel: "Save Default",
    noSuppliers: "No providers configured yet",
    supplierNoApiKey: "No API key configured — click Edit to add one",
    deleteSupplier: "Delete",
    editSupplier: "Edit",
    cancelEdit: "Cancel",
    saveSupplier: "Save",
    notifySection: "Telegram Notifications",
    notifyHint:
      "Send a notification to this Telegram user via the job's linked account when a job finishes. Accepts username, @username, or a t.me link.",
    labelNotifyUsername: "Notification target",
    notifyUsernamePlaceholder: "@username, username, or https://t.me/username",
    labelNotifyEvents: "Notify on",
    notifyEventFailed: "Failed",
    notifyEventSuccess: "Success",
    adminCreds: "Admin Credentials",
    credSaved:
      "Credentials updated. Re-login if you changed your username or password.",
    credFailed: "Update failed",
    labelNewUsername: "New Username",
    labelNewPassword: "New Password",
    labelCurrentPass: "Current Password",
    hintKeepBlank: "— leave blank to keep current",
    currentPassRequired: "Current password is required",
    updateBtn: "Update Credentials",
    importExport: {
      title: "Import / Export",
      exportBtn: "Export Data",
      exportHint: "Export all accounts, jobs and settings (includes sensitive data)",
      exportWarning:
        "The export file contains sensitive data (Telegram session tokens, API hashes, API keys). Keep it safe and do not share it. Proceed?",
      importBtn: "Import",
      importLabel: "Select backup file (.json)",
      importMode: "Import mode",
      modeMerge: "Merge",
      modeMergeHint: "Keep existing data; new accounts (deduplicated by phone) and jobs are appended",
      modeReplace: "Replace all",
      modeReplaceHint: "Delete all existing accounts and jobs, then import",
      replaceWarning: "Replace mode will delete all existing accounts and jobs. Proceed?",
      importing: "Importing...",
      importSuccess: "Import complete: {a} account(s), {t} template(s), {j} job(s), {s} setting(s)",
      importFailed: "Import failed",
      noFileSelected: "Please select a file",
      invalidFile: "Invalid file format",
    },
  },
};

const messages = { zh, en };

export const locale = ref<Locale>(
  (localStorage.getItem("bemby-locale") as Locale | null) ?? "zh",
);

export function setLocale(l: Locale) {
  locale.value = l;
  localStorage.setItem("bemby-locale", l);
}

export function t(key: string): string {
  const parts = key.split(".");
  let cur: unknown = messages[locale.value];
  for (const p of parts) {
    if (cur == null || typeof cur !== "object") return key;
    cur = (cur as Record<string, unknown>)[p];
  }
  return typeof cur === "string" ? cur : key;
}

export function ta(key: string): string[] {
  const parts = key.split(".");
  let cur: unknown = messages[locale.value];
  for (const p of parts) {
    if (cur == null || typeof cur !== "object") return [];
    cur = (cur as Record<string, unknown>)[p];
  }
  return Array.isArray(cur) ? (cur as string[]) : [];
}
